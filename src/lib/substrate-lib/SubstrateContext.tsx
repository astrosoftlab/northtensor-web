...
const reducer = (state: any, action: any) => {
  switch (action.type) {
    ...
    case 'SET_BALANCE_SIGFIGS':
      return { ...state, balanceSigFigures: action.payload }
    default:
...
const loadAccounts = (state: any, dispatch: any) => {
  ...
  const asyncLoadAccounts = async () => {
    try {
      ...
      let allAccounts: any[] = await web3Accounts()
      ...
      const isDevelopment =
        showDevWallets && (systemChainType.isDevelopment || systemChainType.isLocal || isTestChain(systemChain))
...
const SubstrateContextProvider = (props: any) => {
  ...
  function setBalanceSigFigures(n: any) {
    dispatch({ type: 'SET_BALANCE_SIGFIGS', payload: n })
  }

  return (
    <SubstrateContext.Provider value={{ state, setCurrentAccount, setBalanceSigFigures }}>
      {props.children}
